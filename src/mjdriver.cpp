...
void Controller::handle_ready_read()
{
    ...
    if (!readData.isEmpty() && readData[0] == '{')
    {
        // Parse JSON response using Qt (no external dependency)
        QJsonParseError parseError;
        QJsonDocument doc = QJsonDocument::fromJson(readData, &parseError);
        if (parseError.error == QJsonParseError::NoError)
        {
            emit response(QString::fromUtf8(doc.toJson(QJsonDocument::Indented)));
        }
        else
        {
            emit error(QString("Failed to parse JSON: %1").arg(parseError.errorString()));
        }
    }
    else
    {
...
void Controller::reconstructed_bitmap(int headIdx, const QByteArray &imageData, int width, int height)
{
    ...
    QString filePath = QDir(QStandardPaths::writableLocation(QStandardPaths::HomeLocation)).filePath(
        QString("debugBitmap%1.bmp").arg(headIdx));
    image.save(filePath);
...
void Controller::create_bitmap_lines(int numLines, int width)
{
    ...
    QString filePath = QDir(QStandardPaths::writableLocation(QStandardPaths::HomeLocation)).filePath("currentBitmap.bmp");
    image.save(filePath);
...
void Controller::createBitmapTestLines(int numberOfLines,  int lineSpacing, int dropletSpacing, int frequency, int lineLength, int number)
{
    ...
    QImage image = bitmap.toImage();
    QString filePath = QDir(QStandardPaths::writableLocation(QStandardPaths::HomeLocation)).filePath(
        QString("%1testBitmap_%2_%3.bmp").arg(formattedNumber, formattedSpeed, formattedFrequency));
    image.save(filePath);
